...
[2026-06 수정] 누진제 전기요금에 ① 슈퍼유저요금(동·하계 1,000kWh 초과분
736.2원/kWh)과 ② 증분비용 방식(HP가 일으킨 한계 비용만 HP에 귀속)을 반영.
엑셀 전기요금 블록은 명세/검증용 참고로 두고, 실제 결과는 본 코드에서 산출.
"""
import os
import pandas as pd
...
def calc_progressive_billing(usage_kwh, month, solar_offset=0):
    ...
    [2026-06 추가] 슈퍼유저요금 반영 (엑셀 전기요금!B11·B20 명세):
      동계(12·1·2월)·하계(7·8월)에 한해 월 1,000kWh 초과분은 736.2원/kWh 적용.
      히트펌프 난방으로 겨울 사용량이 1,000kWh를 넘는 경우를 정확히 반영하기 위함.
