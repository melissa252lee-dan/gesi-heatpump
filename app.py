...
with col_sim:
    fuel_inflation = st.slider(
        "가스/등유요금 인상률 (%)", 0.0, 15.0, 4.41, step=0.01,
        help="최근 10년(2016–2025년) 주택용 도시가스 연평균 인상률 4.41%를 기본값으로 설정했습니다. 자유롭게 조정하세요.",
    )
    elec_inflation = st.slider(
        "전기요금 인상률 (%)", 0.0, 15.0, 3.03, step=0.01,
        help="최근 10년(2016–2025년) 주택용 전기 연평균 인상률 3.03%를 기본값으로 설정했습니다. 자유롭게 조정하세요.",
    )
...
with col_opt:
    ...
    st.caption(
        "ℹ️ 지원 대상 히트펌프(부속설비, 설치비 포함) 1대당 평균단가 최대 1,400만원 중 "
        "**국비(40%)와 지방비(30%)를 합한 70%를 지원**하며, 자부담은 30%(최대 420만원)예요. "
        "2026년에는 보급 초기 단계로 **제주·전남·경남 등 온난 지역**에서, "
        "태양광 발전시설(3kW 이상)을 설치했거나 설치 예정인 **단독·연립주택**을 대상으로 지원해요. "
        "지자체별 물량·조건이 다르니 거주 지역의 공고를 꼭 확인하세요."
    )
...
사용 중인 요금제를 선택해 주세요.
    ...
    if tariff_choice_simple == "누진제":
        st.caption(
            "ℹ️ 누진제는 많이 쓸수록 단가가 오르는 요금제예요. 히트펌프로 늘어난 전기는 단가 높은 "
            "**누진 상위구간**에서 요금이 붙고(여름·겨울 월 1,000kWh 초과분은 **슈퍼유저요금** 736.2원/kWh), "
            "그 '늘어난 만큼'을 난방요금으로 계산해요."
        )
