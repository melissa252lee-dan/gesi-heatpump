...
def load_simulation_data():
    try:
        try: df_temp_raw = pd.read_csv("외기온도_시간분포.csv", encoding="utf-8", header=None)
        except: df_temp_raw = pd.read_csv("외기온도_시간분포.csv", encoding="cp949", header=None)
        try: df_cop_raw = pd.read_csv("COP_계산기.csv", encoding="utf-8", header=None)
        except: df_cop_raw = pd.read_csv("COP_계산기.csv", encoding="cp949", header=None)
        ...
        for idx, row in df_temp_raw.iterrows():
            ...
            if val.startswith("▶"):
                ...
                if current_reg: regions_temp[current_reg] = []
            if current_reg and val.isdigit() and 0 <= int(val) <= 23:
                hourly_temps = [float(x) if pd.notna(x) else 0.0 for x in row[1:13].values]
        ...
        if header_idx != -1:
            ...
            for _, row in df_data.iterrows():
                ...
                if pd.notna(row[0]) and val not in ["", "nan", "지역"]:
                    try: region_cop_data[val] = {'scop': float(row[15])}
                    except: continue
        return regions_temp, region_cop_data
    except Exception as e:
        st.error(f"🚨 데이터 로드 오류: {e}"); return None, None
...
def calc_elec_bill(kwh, tariff, season="other", contract_kw=5.0):
    ...
    fee = base + energy + (14.0) * kwh # 기후9+연료5
...
st.markdown("""<div class='info-box'><h4 class='info-title'>💡 솔루션 개요</h4><p class='info-text'>🏠 <b>우리 집 맞춤형 분석:</b> 거주 환경과 에너지를 기반으로 히트펌프 전환 이득을 분석합니다. ⚡ <b>고지서 하나로 OK:</b> 최신 한전 요금표가 내장되어 알아서 계산해 줍니다. 🌱💰</p></div>""", unsafe_allow_html=True)
...
if df_temp and df_cop:
    try:
        ...
        with cl2: st.success(f"**✅ {s_reg} 맞춤 효율**\n# {dynamic_cop}"); st.caption(f"{s_reg} 지역의 온도 분포를 반영한 실제 체감 효율입니다.")
...
if st.session_state.get('analyzed'):
    ...
    # 분석 로직... (이하 생략 - 이전 답변의 분석 및 그래프/엑셀 코드와 동일하게 적용)
    st.write(f"분석 완료! (SCOP {dynamic_cop} 적용)")
    # (여기에 분석 결과 요약, 그래프, 비교표, 엑셀 다운로드 코드가 들어갑니다)