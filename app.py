...
def calc_elec_bill(kwh, tariff, season="other", contract_kw=5.0):
    ...
    return round((fee * (1 + VAT_RATE + FUND_RATE)) / 10_000, 4)
...
def heat_to_hp_kwh(heat_man, boiler_type, cop=HP_COP_WINTER):
    ...
    return round(((heat_man * 10_000) / price_mj) / 3.6 * eff / cop, 1)
...
def load_simulation_data():
    ...
    try:
        ...
        for idx, row in df_temp_raw.iterrows():
            val = str(row[0]).strip()
            if val.startswith("▶"):
                if "중부1" in val: current_reg = "중부1"
                elif "중부2" in val: current_reg = "중부2"
                elif "남부" in val: current_reg = "남부"
                elif "제주" in val: current_reg = "제주"
                if current_reg:
...
<div style='background-color: #f8fafc; border-left: 4px solid #3b82f6; padding: 16px; margin-top: 12px; border-radius: 4px;'>
    ...
    <div style='color: #475569; font-size: 0.95rem;'>
        입력하신 평수(<b>{h_size}평</b>)를 기준으로 할 때, 대략 <b>{tank_size}</b> 용량의 축열조(버퍼탱크)가 필요할 것으로 예상됩니다. 👉 체감상 <b>{tank_ref}</b>가 필요합니다!
    </div>
</div>
""", unsafe_allow_html=True)
...
if df_temp is not None and df_cop is not None:
    try:
        zone_name = map_region_to_zone(s_reg)
        dynamic_cop = df_cop[zone_name]['scop']
        
        # 1년 시간 분포 계산 (각 달의 일수 곱하기)
        days_in_month = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31]
        all_temps = []
        for hour in range(24):
            for month in range(12):
                temp = df_temp[zone_name][hour][month]
                all_temps.extend([round(temp)] * days_in_month[month])
        
        temp_counts = pd.Series(all_temps).value_counts().sort_index()
        
        c_chart, c_text = st.columns([2, 1])
        with c_chart:
            st.markdown(f"**🌡️ {s_reg} ({zone_name} 기준) 연간 시간별 외기온도 분포**")
            st.bar_chart(temp_counts, height=250)
            
        with c_text:
            st.markdown("<br><br>", unsafe_allow_html=True)
            st.success(f"**✅ {s_reg} 맞춤형 효율(SCOP)**\n# {dynamic_cop}")
            st.caption(f"좌측 그래프를 보면 **{s_reg}** 지역이 1년 중 어떤 온도에 가장 오래 머무는지 알 수 있습니다. 이 온도 분포를 반영하여 도출한 실제 체감 효율이 **{dynamic_cop}**입니다.")
    except Exception as e:
        dynamic_cop = HP_COP_WINTER
        st.error(f"데이터 표시 오류: {e}")
else:
    dynamic_cop = HP_COP_WINTER
    st.warning("🚨 깃허브에서 CSV 파일을 불러오지 못했습니다. '외기온도_시간분포.csv'와 'COP_계산기.csv' 파일이 정확히 업로드되었는지 확인해 주세요. (현재 기본 효율 3.0으로 계산 중입니다)")
...
with cv2: w_elec  = st.number_input("동절기(1월) 전기요금 고지서 금액 (만원)", value=6, help="청구서의 실 납부금액을 입력하세요. kWh는 자동 역산됩니다.")
ct1,ct2 = st.columns(2)
with ct1: s_capa = st.number_input("태양광 발전 설비 용량 (kW)", value=3.0, step=0.5)
with ct2: elec_tariff = st.selectbox("적용 전기 요금제", ["최적 자동 산출 (저압 누진제)", "주택용 누진제 (저압)", "주택용 누진제 (고압)", "계시별 요금제 TOU (제주)"])
...
if st.session_state.analyzed:
    ...
    tariff = {"최적 자동 산출 (저압 누진제)": "주택용 누진제 (저압)", "주택용 누진제 (저압)": "주택용 누진제 (저압)", "주택용 누진제 (고압)": "주택용 누진제 (고압)", "계시별 요금제 TOU (제주)": "계시별 요금제 TOU (제주)"}[elec_tariff]
    ...
    ann_elec = sum(calc_elec_bill(base_kwh[m-1], tariff, MONTH_TOU[m] if "TOU" in tariff else MONTH_SEASON[m], hp_kw) for m in months)
    ann_hp_add = sum((calc_elec_bill(base_kwh[m-1]+hp_add_m[m-1], tariff, MONTH_TOU[m] if "TOU" in tariff else MONTH_SEASON[m], hp_kw) - calc_elec_bill(base_kwh[m-1], tariff, MONTH_TOU[m] if "TOU" in tariff else MONTH_SEASON[m], hp_kw)) for m in months)
    ...
    st.markdown('<div class="section-title" style="margin-top:20px;">계산 근거 (자동 추정 결과)</div>', unsafe_allow_html=True)
    ca,cb,cc,cd = st.columns(4)
    with ca: st.markdown(f"<div class='calc-box'><div class='calc-label'>1월 추정 전기사용량</div><div class='calc-value'>{cur_kwh:,.0f} kWh</div><div style='color:#64748b;font-size:0.8rem'>요금 {w_elec}만원 역산 ({tariff})</div></div>", unsafe_allow_html=True)
    with cb: st.markdown(f"<div class='calc-box'><div class='calc-label'>추정 히트펌프 용량</div><div class='calc-value'>{hp_kw} kW</div><div style='color:#64748b;font-size:0.8rem'>{h_size}평 × {h_type[:2]} 환경 계수</div></div>", unsafe_allow_html=True)
    with cc: st.markdown(f"<div class='calc-box'><div class='calc-label'>1월 HP 추가 전력</div><div class='calc-value'>{hp_jan_kwh:,.0f} kWh</div><div style='color:#64748b;font-size:0.8rem'>난방비 {w_heat}만원 역산 (SCOP {dynamic_cop})</div></div>", unsafe_allow_html=True)
    with cd: st.markdown(f"<div class='calc-box'><div class='calc-label'>연간 PV 절감 (추정)</div><div class='calc-value'>{ann_pv:,.1f} 만원</div><div style='color:#64748b;font-size:0.8rem'>{s_reg} {s_capa}kW 기준</div></div>", unsafe_allow_html=True)

    st.markdown('<div class="section-title">분석 결과 요약 (Executive Summary)</div>', unsafe_allow_html=True)
    ...
    with cg1:
        ...
        df_a = pd.DataFrame({"연도":years,"기존 설비 유지":gas_cum,"히트펌프 전환":hp_cum}).melt("연도", var_name="시나리오", value_name="누적비용")
        st.altair_chart(alt.Chart(df_a).mark_area(opacity=0.55).encode(x=alt.X("연도:O", title="경과 연도", axis=alt.Axis(labelAngle=0)), y=alt.Y("누적비용:Q", title="비용 (만원)"), color=alt.Color("시나리오:N", scale=alt.Scale(range=["#94a3b8","#0284c7"]), legend=alt.Legend(orient="top-left", title=None))).properties(height=350), use_container_width=True)

    with cg2:
        st.markdown("**투자 회수 및 현금흐름 (Cash Flow)**")
        df_c = pd.DataFrame({"연도":years,"순수익":net_p, "상태":["흑자 전환" if p>0 else "회수 중" for p in net_p]})
        st.altair_chart(alt.Chart(df_c).mark_bar().encode(x=alt.X("연도:O", title="경과 연도", axis=alt.Axis(labelAngle=0)), y=alt.Y("순수익:Q", title="현금흐름 (만원)"), color=alt.Color("상태:N", scale=alt.Scale(domain=["회수 중","흑자 전환"],range=["#94a3b8","#0284c7"]), legend=alt.Legend(orient="top-left", title=None)), tooltip=[alt.Tooltip("연도:O",title="경과 연도"), alt.Tooltip("순수익:Q",title="순수익 (만원)",format=",")]).properties(height=350), use_container_width=True)
    ...
    st.dataframe(pd.DataFrame({"월": mn, "현재 전기요금 (만원)": bef, "전환 후 전기요금 (만원)": aft, "HP 추가 전력 (kWh)": hpa, "PV 절감 (만원)": pvs}), hide_index=True, use_container_width=True)
    ...
    fw, fb, fi, fk = Font(color="FFFFFF", bold=True), Font(bold=True), Font(color="0000FF", bold=True), Font(color="000000")
    thin = Border(left=Side(style="thin"), right=Side(style="thin"), top=Side(style="thin"), bottom=Side(style="thin"))
    center, right = Alignment(horizontal="center"), Alignment(horizontal="right")

    ws = wb.active; ws.title = "①입력_가정"
    ws.merge_cells("A1:D1"); ws["A1"] = f"히트펌프 경제성 분석 — 입력 가정  ({s_reg} {s_sub})"; ws["A1"].fill = hf; ws["A1"].font = fw; ws["A1"].alignment = center
    ws["A2"] = "※ 파란색 셀은 직접 수정 가능합니다. 수정 후 시트2·3 결과가 자동 연동됩니다."; ws["A2"].font = Font(color="0369A1", italic=True)

    for ci, h in enumerate(["항목", "값", "단위", "비고"], 1):
        c = ws.cell(row=3, column=ci, value=h); c.fill = PatternFill(start_color="F1F5F9", end_color="F1F5F9", fill_type="solid"); c.font = fb; c.border = thin

    input_rows = [
        ("동절기 난방비", w_heat, "만원", "1월 기준 직접 입력"), ("동절기 전기요금", w_elec, "만원", "1월 청구서 금액"),
        ("1월 추정 전기사용량", cur_kwh, "kWh", f"{tariff} 역산값"), ("추정 HP 용량", hp_kw, "kW", f"{h_size}평 × 환경계수 자동추정"),
        ("1월 HP 추가 전력", hp_jan_kwh, "kWh", f"SCOP {dynamic_cop} 기준 역산"), ("태양광 설비 용량", s_capa, "kW", "직접 입력"),
        ("연간 PV 발전량", round(sum(reg_pv)*s_capa, 1), "kWh", f"{s_reg} 지역 실측 기준"),
        ("화석연료 물가 인상률", f_inf/100, "%", "연평균 복리"), ("전기요금 물가 인상률", e_inf/100, "%", "연평균 복리"),
        ("설비 CAPEX", 600+h_size*10, "만원","설치 전 총비용"), ("정부 보조금", nat_sub, "만원", "무상 보조"),
        ("지자체 보조금", loc_sub, "만원", "매칭 보조"), ("순 CAPEX (보조금 차감)", net_cap, "만원", "=B13-B14-B15"),
        ("기존 설비 교체비", rep_cost, "만원", "10년차 발생"), ("기후환경요금", CLIMATE_FEE, "원/kWh","한전 고시"),
        ("연료비조정요금", FUEL_ADJ, "원/kWh","한전 상한 기준"), ("부가세", VAT_RATE, "%", ""), ("전력산업기반기금", FUND_RATE, "%", "2025.7 인하 후"),
    ]
    for ri, (name, val, unit, note) in enumerate(input_rows, 4):
        ws.cell(row=ri, column=1, value=name).border = thin
        vc = ws.cell(row=ri, column=2, value=val); vc.font = fi; vc.border = thin; vc.alignment = right
        ws.cell(row=ri, column=3, value=unit).border = thin; ws.cell(row=ri, column=4, value=note).border = thin
    ws["B16"] = "=B13-B14-B15"; ws["B16"].font = fk; ws["B16"].border = thin
    for r in [11, 12, 20, 21]: ws.cell(row=r, column=2).number_format = "0.0%"
    for col, width in zip(["A","B","C","D"], [26,16,10,28]): ws.column_dimensions[col].width = width

    ws2 = wb.create_sheet("②월별_전기요금")
    ws2.merge_cells("A1:F1"); ws2["A1"] = "월별 전기요금 상세 비교 (히트펌프 전환 전·후)"; ws2["A1"].fill = hf2; ws2["A1"].font = fw; ws2["A1"].alignment = center
    for ci, h in enumerate(["월", "현재 전기요금(만원)", "전환 후 전기요금(만원)", "절감액(만원)", "HP 추가전력(kWh)", "PV 절감(만원)"], 1):
        c = ws2.cell(row=2, column=ci, value=h); c.fill = PatternFill(start_color="F1F5F9", end_color="F1F5F9", fill_type="solid"); c.font = fb; c.border = thin; c.alignment = center
    for ri, (a, b, c, d, e) in enumerate(zip(mn, bef, aft, hpa, pvs), 3):
        saving = round(b - c, 2)
        for ci, v in enumerate([a, b, c, saving, d, e], 1):
            cell = ws2.cell(row=ri, column=ci, value=v); cell.border = thin
            if ci in [2, 3, 4, 6]: cell.number_format = "0.00"
            if ci == 4 and saving > 0: cell.font = Font(color="0284C7", bold=True)
    ws2.cell(row=15, column=1, value="연간 합계").font = fb; ws2.cell(row=15, column=1).border = thin
    for ci, col in enumerate(["B","C","D","E","F"], 2): c = ws2.cell(row=15, column=ci, value=f"=SUM({col}3:{col}14)"); c.font = fk; c.border = thin; c.number_format = "0.00"
    for col_letter, w in zip(["A","B","C","D","E","F"], [8,16,18,12,14,12]): ws2.column_dimensions[col_letter].width = w

    ws3 = wb.create_sheet("③15년_재무시뮬레이션")
    ws3.merge_cells("A1:G1"); ws3["A1"] = "15년 장기 재무 시뮬레이션 — 입력 가정 시트 연동형"; ws3["A1"].fill = hf; ws3["A1"].font = fw; ws3["A1"].alignment = center
    ws3["A2"] = "※ 입력값은 [①입력_가정] 시트에서 수정하세요. 아래 수식이 자동 재계산됩니다."; ws3["A2"].font = Font(color="0369A1", italic=True)
    for ci, h in enumerate(["경과연도", "기존설비 연간 OPEX(만원)", "히트펌프 연간 TCO(만원)", "연간 절감액(만원)", "기존설비 누적 OPEX(만원)", "히트펌프 누적 TCO(만원)", "누적 순이익 NPV(만원)"], 1):
        c = ws3.cell(row=3, column=ci, value=h); c.fill = PatternFill(start_color="1E293B", end_color="1E293B", fill_type="solid"); c.font = fw; c.border = thin; c.alignment = center
    REF = "\'①입력_가정\'!"
    for y in range(1, 16):
        r, prev = y + 3, y + 2
        fi_formula, ei_formula = f"(1+{REF}B11)^{y-1}", f"(1+{REF}B12)^{y-1}"
        rep_formula = f"+{REF}B17" if y == 10 else ""
        ann_gas_f = f"=({REF}B4*6.5*{fi_formula})+({REF}B5*12*{ei_formula}){rep_formula}"
        ann_hp_f = f"=({REF}B5*12*{ei_formula})+({REF}B5*2.5*{ei_formula})-({REF}B7/10000*200*{ei_formula})"
        ws3.cell(row=r, column=1, value=y).border = thin
        c_gas = ws3.cell(row=r, column=2, value=ann_gas_f); c_gas.font = fk; c_gas.border = thin; c_gas.number_format = "#,##0"
        c_hp = ws3.cell(row=r, column=3, value=ann_hp_f); c_hp.font = fk; c_hp.border = thin; c_hp.number_format = "#,##0"
        c_sav = ws3.cell(row=r, column=4, value=f"=B{r}-C{r}"); c_sav.font = fk; c_sav.border = thin; c_sav.number_format = "#,##0"
        cum_g_f, cum_h_f = (f"=B{r}", f"={REF}B16+C{r}") if y == 1 else (f"=E{prev}+B{r}", f"=F{prev}+C{r}")
        c_cg = ws3.cell(row=r, column=5, value=cum_g_f); c_cg.font = fk; c_cg.border = thin; c_cg.number_format = "#,##0"
        c_ch = ws3.cell(row=r, column=6, value=cum_h_f); c_ch.font = fk; c_ch.border = thin; c_ch.number_format = "#,##0"
        c_np = ws3.cell(row=r, column=7, value=f"=E{r}-F{r}"); c_np.font = fk; c_np.border = thin; c_np.number_format = "#,##0"
        if net_p[y-1] > 0: c_np.fill = PatternFill(start_color="DBEAFE", end_color="DBEAFE", fill_type="solid")
    ws3.cell(row=19, column=1, value="15년 합계").font = fb; ws3.cell(row=19, column=1).border = thin
    for ci, col in enumerate(["B","C","D"], 2): c = ws3.cell(row=19, column=ci, value=f"=SUM({col}4:{col}18)"); c.font = fk; c.border = thin; c.number_format = "#,##0"
    for col_letter, w in zip(["A","B","C","D","E","F","G"], [8,16,16,14,18,18,18]): ws3.column_dimensions[col_letter].width = w
    ws3.row_dimensions[3].height = 42

    buf = io.BytesIO(); wb.save(buf)
    st.download_button("📊 수식 연동 인터랙티브 엑셀 다운로드", data=buf.getvalue(), file_name=f"HeatPump_Analysis_{s_reg}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")