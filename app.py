...
.help-text { color: #64748b; font-size: 0.85rem; margin-top: -10px; margin-bottom: 10px; }
</style>
""", unsafe_allow_html=True)

# (지역 데이터 및 pv_monthly_data 생략 - 기존과 동일하게 유지)
regions_full = {
    ...
    "세종": ["세종특별자치시"], "대전": ["동구","중구","서구","유성구","대덕구"], "대구": ["중구","동구","서구","남구","북구","수성구","달서구","달성군"],
    "부산": ["중구","서구","동구","영도구","부산진구","동래구","남구","북구","해운대구","사하구","금정구","강서구","연제구","수영구","사상구","기장군"],
    "울산": ["중구","남구","동구","북구","울주군"], "광주": ["동구","서구","남구","북구","광산구"],
...
def calc_elec_bill(kwh, tariff="주택용 누진제 (저압)"):
    ...
    energy = (kwh * e) if kwh <= 300 else (300*120 + (kwh-300)*214) 
    fee = (b + energy + 14.0 * kwh) * 1.127
...
with cs2:
    ...
    is_s = s_reg in ["제주도","경상남도","전라남도"]
    sub_loc = st.checkbox("지자체 매칭 보조금 적용 (280만원)", value=is_s)
    ...
    # 체크박스 형태의 선택 (하나만 선택 가능하도록 로직 구현)
    t1 = st.checkbox("주택용 누진제 (저압)", value=True)
    t2 = st.checkbox("주택용 누진제 (고압)", value=False)
    t3 = st.checkbox("계시별 요금제 TOU (제주)", value=False)
    
    # 실제 계산에 쓰일 tariff 결정
    if t3: elec_tariff = "계시별 요금제 TOU (제주)"
    elif t2: elec_tariff = "주택용 누진제 (고압)"
    else: elec_tariff = "주택용 누진제 (저압)"
...
if st.session_state.analyzed:
    ...
    ann_heat = w_heat * 4.8; ann_elec = calc_elec_bill(cur_k, elec_tariff) * 12
    ann_hp_net = (ann_heat / dynamic_cop) * 0.85

    years, gas_cum, hp_cum, net_p = list(range(1,16)), [], [], []
    g_s, h_s, pb = 0.0, float(net_cap), "15년 초과"
    for y in years:
        cg = ann_heat * ((1+f_inf/100)**y) + ann_elec
        ch = ann_hp_net * ((1+e_inf/100)**y) + ann_elec
    ...
    rows1 = [("항목", "값", "단위", "비고"), ("1월 난방비", w_heat, "만원", ""), ("1월 전기요금", w_elec, "만원", ""), ("지역 sCOP", dynamic_cop, "-", "기상연동"), ("설비 CAPEX", 600+h_size*10, "만원", ""), ("정부보조금", 560 if sub_nat else 0, "만원", ""), ("지방보조금", 280 if sub_loc else 0, "만원", ""), ("순 투자비", net_cap, "만원", "=B7-B8-B9"), ("가스 인상률", f_inf/100, "%", ""), ("전기 인상률", e_inf/100, "%", "")]
